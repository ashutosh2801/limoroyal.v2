...
function ReturnTrip() {
  ...
  return (
    <div>
        <h2 className="text-lg xl:text-xl font-bold mb-4">Your Return Trip</h2>
        ...
        <div className="border border-gray-200 rounded-xl p-3 xl:p-4">
            ...
            <div className="flex justify-between items-center">
                ...
                <div>
                    {/* Locations */}
                    <Locations data={data.returnData} />            
                </div>  

                <button 
                onClick={() => router.push("/passengers-info")}
        ...
        <div className="mt-6 border border-gray-200 rounded-xl p-3 xl:p-4">
            <div className="flex justify-between">
                <div>
                <p className="font-semibold text-xs md:text-sm">{data.returnData?.selectedVehicle.name}</p>
                {data.returnData?.selectedVehicle.desc && (
                <div className="flex items-center gap-4 mt-3 text-xs text-gray-700">
                    <p className="flex items-start md:items-center gap-1">
                        <ExclamationCircleIcon className="w-5 h-5 flex-shrink-0 " /> <span dangerouslySetInnerHTML={{ __html: data.returnData?.selectedVehicle.desc }} />
                    </p>
                </div>
                )}                                
                </div>
                ...
                <div className="flex items-end flex-col md:flex">
                    ...
                    <Image
                        src={data.returnData?.selectedVehicle.img ?? vClass}
                        alt={data.returnData?.selectedVehicle.name}
        ...
        <div className="mt-7">
            ...
            <div className="border border-gray-200 rounded-xl bg-white overflow-hidden">
                ...
                <div>                                    
                    ...
                    <p className="text-xs xl:text-sm font-semibold mt-1">{`${data.returnData?.PickupInfo.title} ${data.returnData?.PickupInfo.firstName} ${data.returnData?.PickupInfo.lastName}`}</p>
                    <p className="text-xs xl:text-sm text-gray-500 mt-4">Contact details</p>
                    <p className="text-xs xl:text-sm font-medium mt-1">
                    {`${data.returnData?.PickupInfo.email} • +${data.returnData?.PickupInfo.contactNumber}`}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <p className="text-xs md:text-sm font-semibold mt-1">{data.returnData?.PickupInfo.trip_notes || "-"}</p>
                </div>
                </div>                             
